...
\section{\color{red}Signaling}
  ...
  WebRTC doesn't implement signaling, different applications may need different protocols, there is no single answer that fits all problems. Amongst multiple options, signaling can be done by using SIP, XMPP, WebSockets, Socket.io or by implementing a custom protocol.

  %https://bloggeek.me/siganling-protocol-webrtc/
  {\color{red}[SIP]}

  SIP is a signaling method, it allows web browsers to have audio, video and SMS capabilities like mobile phones. For instance, it's possible to interoperate with SIP, mobile and fixed phones.

  SIP is used in VoIP applications duo its compatibility with PSTN. Service providers are disponibilizing access to their SIP infrastrucures through WebSockets. Frameworks like JSSip or sipML5 are used on client side to encode SIP messages, making SIP accessible to web based applications. 

  {\color{red}[XMPP]}

  HTTP works on a request followed by a server response. Sometimes it's required that some informations are obtained in real time, as we saw, some NAT's don't suport callbacks from servers, one technique to overcome this problem is polling.
  ...
  XMPP's bi-directional communication is achieved through BOSH, which basically consists on long polling.
