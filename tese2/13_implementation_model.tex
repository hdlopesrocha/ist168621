...
%RP mudei ``most important'' para critical
...
The identification number by itself is not sufficient to identify an object, as objects from different types can have the same identification number. In order to solve this problem, when an object is created, its correspondent data must contain the owner's object type. 
...
\begin{itemize}
    ...
    \item{\emph{\{groupId,sessionId,end\}} $\rightarrow$ for finding intersecting chunks with a given time within a session}
    \item{\emph{\{groupId,owner,end\}} $\rightarrow$ for finding intersecting chunks with a given time within a user (for a different session) or group}
